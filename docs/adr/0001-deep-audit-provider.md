...
- Store non-secret endpoint and model preferences in the app configuration
  directory, never in a project or Skill directory.
...
- Show the exact endpoint, model, and eligible file list before every run.
  `SKILL.md` is required; the user may deselect other eligible text files.
...
requires a new adapter, not changes to the Audit evidence interface.
